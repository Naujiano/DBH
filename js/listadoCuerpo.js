function preinit(){
	...
	$('body').addClass('DBH')
	function contextmenubuttons (event,btn) {
		var $div = $('<div id="caparightclick" class="divMenuContextual" style="width:auto;height:auto;background:;z-index:99999999999999999;position:absolute;"/>')
		, $capa = $('#caparightclick')
		//, isopen = $capa.is(':visible')
		, $btn = $(btn)
		, $btn1 = $btn.clone().removeClass('color-blue').click(function(){$btn.removeClass('color-blue').attr('clear-setdefaults',0);$div.hide()})
		, $btn2 = $btn.clone().addClass('color-blue').click(function(){$btn.addClass('color-blue').attr('clear-setdefaults',1);$div.hide()})
		, $botones = $btn1.add($btn2)
		...
		$div.width(width)
		//$btn1.css({margin:0,padding:0}).width('100%').find('.fa-caret-down').remove()
		$botones.css({padding:0,'text-align':'center'}).width(width).find('.fa-caret-down').remove()
		$('body').append($div.append($botones))
