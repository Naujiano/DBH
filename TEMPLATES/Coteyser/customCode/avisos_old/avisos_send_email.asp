...
function enviar_email ()
	...
	if interno <> "1" then 
		...
		response.write "a juan"
		email.AddTo "Juan Martin", "juan.martin.m@gmail.com"
	end if
	'success = Mail.SendEmail(email)
	Set task = Mail.SendEmailAsync(email)
	If (task Is Nothing ) Then
	response.write "fail tarea"
		outFile.WriteLine(Mail.LastErrorText)
		WScript.Quit
	End If
	'newBound = UBound(Tasks) + 1
	'redim Tasks (newBound)
		success = task.Run()
		If (success <> 1) Then
	response.write "fail run tarea"
