{
	...
	*/
	...
	customjs.updateOfertas = function () {
		...
		//var porcentaje = prompt ( 'Introducir tanto porciento (%) de reducción de las ofertas caducadas' )
	...
	customjs.insertSerial = function (o) {
		var ids = DBH.area().recsid
		if ( ! confirm ( 'Crear ' + o + ' para los registros seleccionados?' ) ) return false
		//var porcentaje = prompt ( 'Introducir tanto porciento (%) de reducción de las ofertas caducadas' )
		//if ( ! procentaje || !isNaN(porcentaje) ) { alert('Seleccione registros' ); return false }
		if ( ! ids.length ) { alert('Seleccione registros' ); return false }
		//if ( ! prompt ( 'Introducir tanto porciento (sin %) variacion oferta' ) ) return false
		if ( o == "polizas" ) {
			var tablename = "personas_riesgos"
			var pkname = "pr_codpersona"
		}
		if ( o == "riesgos" ) {
			var tablename = "personas_riesgos_riesgos"
			var pkname = "prr_pr_id"
		}
		var sql = "exec dbo.sp_insert_serial '" + ids + "','" + tablename + "','" + pkname + "'" //sp_insert_serial '111882,111879','personas_riesgos','pr_codpersona'
	     , res = DBH.ajax.sql(sql)
		console.log(sql)
		if ( !res ) {
			alerta('Error en la creación de registros.')
		}else{
			alerta('Registros creados correctamente.',1)
		}
		console.log(ids)
	}
	customjs.VS_insertPoliza = function (pr_id) {
		if ( ! confirm ( 'Crear esta póliza en VisualSeg?' ) ) return false
		//console.log("pr_id"+pr_id)
		var sql = "exec sp_vs_insert_poliza " + pr_id
	     , res = DBH.ajax.sql(sql)
		console.log(sql)
		if ( !res ) {
			alerta('Error en la creación del registro en VS.')
		}else{
			alerta('Registro creado correctamente en VS.',1)
		}
	}
	customjs.VS_insertRiesgo = function (prr_id) {
		if ( ! confirm ( 'Crear este riesgo en VisualSeg?' ) ) return false
		//console.log("pr_id"+pr_id)
		var sql = "exec sp_vs_insert_riesgo " + prr_id
	     , res = DBH.ajax.sql(sql)
		console.log(sql)
		if ( !res ) {
			alerta('Error en la creación del registro en VS.')
	...
	customjs.addCoberturas = function (prr_id,coberturas_grupos_id,tipo_cobertura) {
		var ids = DBH.area().recsid
		if ( ! confirm ( 'Crear ' + tipo_cobertura + ' para los riesgos seleccionados?' ) ) return false
		if ( ! ids.length ) { alert('Seleccione registros' ); return false }
		if ( ! coberturas_grupos_id || isNaN ( coberturas_grupos_id ) ) { alert ('Seleccione un grupo de coberturas.') ; return false }
		if ( tipo_cobertura == 'garantias' ) {
			ids.forEach ( function ( id ) {
				let sql = "INSERT INTO prr_garantias ( prg_prr_id, prg_gar_id ) ( SELECT " + id + " , ramgar_gar_id FROM garantias_grupos inner join ramos_garantias on gg_ramgar_id = ramgar_id WHERE gg_grupo_id = " + coberturas_grupos_id + ")"
				, res = DBH.ajax.sql(sql)
				console.log(res)
				if ( ! res ) {
					alerta( 'Error en la insercion de garantias. Riesgo ID = ' + id )
				} else {
					alerta( 'Garantias insertadas. Riesgo ID = ' + id , 1)
				}
				console.log(sql)
			} )
		} else {
			ids.forEach ( function ( id ) {
				let sql = "INSERT INTO prr_capitales ( prc_prr_id, prc_cap_id ) ( SELECT " + id + " , ramcap_cap_id FROM capitales_grupos inner join ramos_capitales on cg_ramcap_id = ramcap_id WHERE cg_grupo_id = " + coberturas_grupos_id + ")"
				, res = DBH.ajax.sql(sql)
				console.log(res)
				if ( ! res ) {
					alerta( 'Error en la insercion de capitales. Riesgo ID = ' + id )
				} else {
					alerta( 'Capitales insertados. Riesgo ID = ' + id , 1)
				}
				console.log(sql)
			} )
		}
	}
	customjs.matriz_replicar = function ( mat_id ) {
		if ( ! confirm ( 'Replicar la matriz seleccionada?' ) ) return false
		let sql = "exec sp_matriz_replicar " + mat_id
		, res = DBH.ajax.sql(sql)
		if ( ! res ) {
			alerta( 'Error en la replicación de la matriz. Matriz ID = ' + mat_id )
		} else {
			alerta( 'Matriz replicada. Matriz ID = ' + mat_id , 1)
		}
	}
}
