...
function esFechaValida (fecha)
{
  ...
  return true
	...
	function esFechaYHoraValida(fecha){
		...
		if ( fechaArr.length < 2 ) { var horaPart = "0:0:0" }
		else { var horaPart = fechaArr[1] }
		var fechaPart = fechaArr[0]
		var fechaArr = fechaPart.split ( "/" )
		if ( fechaArr.length < 3 ) { return false }
		var dia = fechaArr[0]
		var mes = fechaArr[1]
		var ano = fechaArr[2]
		horaArr = horaPart.split ( ":" )
		if ( horaArr.length < 2 ) { return false }
		var hora = horaArr[0]
		var minutos = horaArr[1]
		if ( horaArr.length == 3 ) { var segundos = horaArr[2] } else { var segundos = 0 }
		//alert(horaPart)
    // Compruebo el mes.
    ...
    if (mes == 1 || mes == 3 || mes == 5 || mes == 7 || mes == 8 || mes == 10 || mes == 12)
      ...
      var diaMax = 30
    if (dia.toString() == "" || isNaN(dia) || dia < 1 || dia > diaMax || (dia - Math.round(dia)) != 0) return false
    // Compruebo la hora
    if(isNaN(hora)||hora.toString()==""||hora<0||hora>23)return false
    // Compruebo el minuto
    if(isNaN(minutos)||minutos.toString()==""||minutos<0||minutos>59)return false
    // Compruebo el segundo
    if(isNaN(segundos)||segundos.toString()==""||segundos<0||segundos>59)return false
